...
func (g *gcmAsm) counterCrypt(out, in []byte, counter *[gcmBlockSize]byte) {
	...
	for len(in) >= gcmBlockSize {
		...
		in0 := binary.BigEndian.Uint64(in[0:])
		in1 := binary.BigEndian.Uint64(in[8:])
		m0 := binary.BigEndian.Uint64(mask[:8])
		m1 := binary.BigEndian.Uint64(mask[8:])
		binary.BigEndian.PutUint64(out[:8], in0^m0)
		binary.BigEndian.PutUint64(out[8:], in1^m1)
		out = out[16:]
		in = in[16:]
	}
